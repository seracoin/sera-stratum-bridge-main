...
var walletRegex = regexp.MustCompile("sera:[a-z0-9]+")
...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "sera:") {
		return CleanWallet("sera:" + in)
	}
	...
	return "", errors.New("unable to coerce wallet to valid sera address")
}
